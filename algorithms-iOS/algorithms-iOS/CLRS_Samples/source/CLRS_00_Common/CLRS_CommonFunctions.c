...
void generateDistinctInt32Array(int array[], unsigned int numOfElements, int minValue, int maxValue){
  generateSortedInt32Array(array, numOfElements, minValue);

  int middleIndex = numOfElements >> 1;
  
  for (int i = 0; i <= middleIndex; i++) {
    swap(&array[i], &array[numOfElements - 1 - i]);
  }
}
